function changeImage(type, img_url, outputId, folder) {
    ...
    const selectedOption = Array.from(imgs).map(element => element.src.split('/').pop());

    if (selectedOption.includes(img_url)) {
        document.getElementById(img_url).classList.remove('option-btn-selected');
        document.getElementById(outputId).setAttribute('src', 'images/blank-img.png');
    } else {
        ...
        console.log(`change${type.charAt(0).toUpperCase() + type.slice(1)}`, fullImgUrl);
...
function changeBg(img_url) { changeImage('bg', img_url, 'bg-output', 'Background'); }
function changeWings(img_url) { changeImage('wings', img_url, 'wings-output', 'body-accessories'); }
function changeSkin(img_url) { changeImage('skin', img_url, 'skin-output', ''); }
function changeExpression(img_url) { changeImage('expression', img_url, 'expression-output', 'expression'); }
function changeEyes(img_url) { changeImage('eyes', img_url, 'eyes-output', 'hand-accessories'); }
function changeDress(img_url) { changeImage('dress', img_url, 'dress-output', 'head-accessories'); }
function changeMask(img_url) { changeImage('mask', img_url, 'mask-output', ''); }
function changeoutfit(img_url) { changeImage('Outfit', img_url, 'hair-output', 'Outfit'); }
function changeHorn(img_url) { changeImage('horn', img_url, 'horn-output', ''); }
function changesidecharactor(img_url) { changeImage('side-characters', img_url, 'wig-output', 'side-characters'); }
...
function download() {
    ...
    html2canvas(element, {
        ...
        const scale = Math.min(highResCanvas.width / canvas.width, highResCanvas.height / canvas.height);
...
document.getElementById('shareTelegramButton').addEventListener('click', async function () {
    const canvas = document.getElementById('combined-output-img');
    const imageUrl = canvas.toDataURL('image/png').split(',')[1]; // Get base64 part of the URL
